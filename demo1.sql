...
SELECT
       ...
       avg_vote
FROM movies
WHERE date_published BETWEEN '2017-12-25' AND '2018-01-01'
      AND votes >= 10000
...
FROM movies
WHERE
      description LIKE '%zombie%'
      AND worlwide_gross_income IS NOT NULL
ORDER BY
        CAST(LTRIM(worlwide_gross_income,'$') AS INT) DESC
LIMIT 5;
...
SELECT
       name,
       height AS height_cm,
       FLOOR(height/(2.54*12)) ||' feet '||
           ROUND(ROUND(height/(2.54*12),2) % 1 * 12,0) ||' inches'
           AS height_ft
FROM actors
...
SELECT
       ...
       worlwide_gross_income,
       ROUND( (
           (CAST(LTRIM(usa_gross_income, '$') AS DECIMAL) / CAST(LTRIM(worlwide_gross_income, '$') AS DECIMAL))
           )*100,1)
           AS pct_gross_income_usa
FROM movies
WHERE actors LIKE '%Tom Hanks%'
ORDER BY
        pct_gross_income_usa
LIMIT 10;
